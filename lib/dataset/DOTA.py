...
class DOTA_custom(IMDB):
    def __init__(self, image_set, root_path, data_path, result_path=None, mask_size=-1, binary_thresh=None):
        ...
        """
        self.image_set = image_set
        super(DOTA_custom, self).__init__('DOTA_custom', self.image_set, root_path, data_path, result_path)  # set self.name
    ...
    def load_image_set_index(self):
        ...
        :return:
        """
        image_lists = glob(os.path.join(self.data_path, 'images/images', "*.png"))
        image_lists = [image.split('images/images/')[1][:-4] for image in image_lists]
        return image_lists
    ...
    def image_path_from_index(self, index):
        ...
        # TODO: when data ready, the entrance here should be changed
        # image_file = os.path.join(self.data_path, self.image_set, index)
        image_file = os.path.join(self.data_path, 'images/images', index + '.png')
        assert os.path.exists(image_file), 'Path does not exist: {}'.format(image_file)
        return image_file
    ...
    def load_annotation(self, index):
        ...
        # f = codecs.open(filename, 'r', 'utf-16')
        if self.image_set == 'TrainingSet':
            filename = os.path.join(self.data_path, 'labelTxt-v1.0/trainset_reclabelTxt', index + '.txt')
            f = codecs.open(filename, 'r')
            objs = f.readlines()[2:]
            objs = [obj.strip().split(' ') for obj in objs]
            # objs = tree.findall('object')
